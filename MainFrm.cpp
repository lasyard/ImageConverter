...
void CMainFrame::OnConvert() 
{
	...
	ASSERT(pChildFrame);
	CImageConverterView *pImageConverterView =
		(CImageConverterView *)(pChildFrame->GetActiveView());
	ASSERT(pImageConverterView);
	CImageConverterDoc *pImageConverterDoc =
		(CImageConverterDoc *)(pImageConverterView->GetDocument());
	ASSERT(pImageConverterDoc);
	...
	if (cvrt_dlg.DoModal() == IDOK) {
		...
		if (index > 0) path_name = path_name.Left(index);
		path_name += '.';
