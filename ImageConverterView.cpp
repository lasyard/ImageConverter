...
GLshort *CImageConverterView::render_image(jas_image_t *image)
{
	...
	if ((cmptlut[0] = jas_image_getcmptbytype(alt_image,
		JAS_IMAGE_CT_COLOR(JAS_CLRSPC_CHANIND_RGB_R))) < 0 ||
		(cmptlut[1] = jas_image_getcmptbytype(alt_image,
		JAS_IMAGE_CT_COLOR(JAS_CLRSPC_CHANIND_RGB_G))) < 0 ||
		(cmptlut[2] = jas_image_getcmptbytype(alt_image,
	...
	for (i = 1; i < 3; i++) {
		if (jas_image_cmptwidth(alt_image, cmptlut[i]) != iw ||
			jas_image_cmptheight(alt_image, cmptlut[i]) != ih) {
			jas_cmprof_destroy(outprof);
			jas_image_destroy(alt_image);
			return NULL;
		}
...
void CImageConverterView::change_view(const int cx, const int cy)
{
	...
	glLoadIdentity();
	glRasterPos2i((cx > m_vw) ? (cx-m_vw)/2 : 0,
		(cy > m_vh) ? (cy-m_vh)/2 : 0);
}
