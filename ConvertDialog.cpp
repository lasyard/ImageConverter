...
CConvertDialog::CConvertDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CConvertDialog::IDD, pParent)
	, m_caption (_T(""))
{
...
{
	m_fmt = fmt;
	jas_image_fmtinfo_t *fmtinfo = jas_image_lookupfmtbyid(m_fmt);
...
BOOL CConvertDialog::OnInitDialog() 
{
	...
	if (m_caption) SetWindowText(m_caption);
	for (int i = 0; i < JAS_IMAGE_MAXFMTS; i++) {
		if (i != m_fmt) {
			if (!(fmtinfo = jas_image_lookupfmtbyid(i))) break;
